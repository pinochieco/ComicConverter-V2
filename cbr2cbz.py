...
class MainWindow(QMainWindow):
    ...
    def _scansiona(self):
        ...
        for f in files_trovati:
            ...
            if kb > 1024:
                self.lista_file.addItem(f"{f.name}  ({kb//1024}.{kb%1024//100} MB)")
            else:
                self.lista_file.addItem(f"{f.name}  ({kb} KB)")
    ...
    def _avvia(self):
        ...
        # Prepara convertitore
        conv = Convertitore(self.segnali)
        t = threading.Thread(target=conv.converti,
                             args=(self.files_list, formato, elimina, self.cartella_output),
                             daemon=True)
        t.start()
        ...
        for i in range(self.lista_file.count()):
            ...
            if item and nome in item.text() and not item.text().startswith("Nessun"):
                if stato == "Elaborazione...":
                    ...
                    item.setText(item.text().split("  (")[0] + "  [OK]")
                    item.setForeground(QColor("#a6e3a1"))
                elif stato.startswith("ERRORE"):
                    item.setText(item.text().split("  (")[0] + "  [ERRORE]")
